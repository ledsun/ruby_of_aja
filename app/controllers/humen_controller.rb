class HumenController < ApplicationController
  ...
  # GET /humen/1.json
  def show
    @solution = CodeRay.scan(@human.solution, :ruby).div(:line_numbers => :table)
  end
  ...
  private
    ...
    def human_params
      params.require(:human).permit(:problem, :solution, :aim, :expected_time, :order, :category_id, :source_type)
    end
end
